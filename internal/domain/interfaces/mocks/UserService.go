...
// ChangeUserPfp provides a mock function with given fields: ctx, userId, newImage
func (_m *UserService) ChangeUserPfp(ctx context.Context, userId uuid.UUID, newImage *multipart.FileHeader) error {
	ret := _m.Called(ctx, userId, newImage)

	if len(ret) == 0 {
		panic("no return value specified for ChangeUserPfp")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, uuid.UUID, *multipart.FileHeader) error); ok {
		r0 = rf(ctx, userId, newImage)
	} else {
		r0 = ret.Error(0)
	}
...
func (_m *UserService) DeleteUserPfp(ctx context.Context, userId uuid.UUID) error {
