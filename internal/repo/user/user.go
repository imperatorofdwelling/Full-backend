...
func (r *Repository) CheckUserExists(ctx context.Context, email string) (bool, error) {
	...
	stmt, err := r.Db.PrepareContext(ctx, "SELECT EXISTS (SELECT 1 FROM users WHERE email = $1), email")
	if err != nil {
...
func (r *Repository) FindUserByID(ctx context.Context, id uuid.UUID) (*user.User, error) {
	...
	stmt, err := r.Db.PrepareContext(ctx, "SELECT * FROM users WHERE id = $1, id")
	if err != nil {
